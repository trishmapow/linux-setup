...
# MODIFIED BY github/trishmapow 2019
# ROUNDED TO NEAREST DIGIT AND PADDED OUTPUT TO STOP PANEL MOVING
...
function to_human_readable_output () {
  ...
  BANDWIDTH=$(awk '{printf "%.0f", $1}' <<< "${BANDWIDTH}")
  ...
  BANDWIDTH=$(awk '{printf "%10s", $0}' <<< "${BANDWIDTH}")
...
INFO+="↓<span weight='Bold' fgcolor='White'> ${RX}</span>"
INFO+=" "
INFO+="↑<span weight='Bold' fgcolor='White'> ${TX}</span>"
INFO+="</txt>"
